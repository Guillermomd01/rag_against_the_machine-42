...
class Retriever():
    ...
    def retrieve(
        self, query: str, indexer: Indexer,
            top_k: int = 5) -> list[MinimalSource]:
        """Retrieves the most relevant chunks based on the query."""
        scores = self._calculate_similarities(query, indexer)
        top_scores = scores[:top_k]
        results = [indexer.list_minimal_sources[index]
                   for index, _ in top_scores]
        return results
    ...
    def _calculate_similarities(
        self, query: str,
            indexer: Indexer) -> List[tuple[int, float]]:
        """Retrieves the most relevant chunks based on the query."""
